...
export async function GET(request: Request) {
  try {
    ...
    if (assessmentId) {
      const snapshot = await getModuleSnapshot(assessmentId, 'DATA');
      ...
      if (!hasSnapshot) {
        const python = await proxyToPythonIfAvailable(request, '/data-foundation');
        if (python) return python;
      }
    }

    const payload = await getQuestionModulePayload(DATA_FOUNDATION_LIBRARY, assessmentId);
    return NextResponse.json(payload);
  } catch (error) {
    console.error(error);
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to load Data Foundation.' }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const payload = await fastWriteQuestionScore(DATA_FOUNDATION_LIBRARY, questionId, score, assessmentId);
    if (assessmentId) scheduleRecompute(assessmentId, 'DATA');
    return NextResponse.json({ ok: true, data: payload });
  } catch (error) {
    console.error(error);
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to save Data Foundation score.' }, { status: 500 });
  }
}
